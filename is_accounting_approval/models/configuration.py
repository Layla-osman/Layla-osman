...
# inherit to add F.M Limit to approve finance approval requeset
class TResCompany(models.Model):
    _inherit = 'res.company'

    f_limit = fields.Float('FC Manager Limit')
    doa_currency = fields.Many2one('res.currency', 'DOA Currency', default=lambda self: self.env.user.company_id.currency_id)
