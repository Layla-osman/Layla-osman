...
def amount_to_text_arabic(number, currency):
    ...
    # raise Warning(_(cents_name))
    if start_word and end_word != "صفر":
        final_result = u'فقط '+start_word+' '+_get_currency_name_by_code(currency)[0]+' '+u'و'+' '+end_word+" "+cents_name
    if start_word and end_word == "صفر":
        final_result = ''
    if start_word != "صفر" and end_word == "صفر":
        final_result = u'فقط ' + start_word + ' ' + _get_currency_name_by_code(currency)[0]
    if start_word == "صفر" and end_word != "صفر":
        final_result = u'فقط ' + end_word + " " + cents_name
    return final_result